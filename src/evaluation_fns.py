...
def fakenews_accuracy(predictions,targets,mask):
  # targets = batch_size * seq_len (each token is labeled with the same stance), so we just need the first one
  actual_targets = targets[:,0]
  # Ignore mask as it is per token and we are classifying the whole sentence
  with tf.name_scope('fakenews_accuracy'):
    return tf.metrics.accuracy(labels=actual_targets, predictions=predictions)
...
dispatcher = {
  ...
  'fakenews_accuracy': fakenews_accuracy
}

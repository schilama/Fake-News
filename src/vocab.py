...
class Vocab:
  ...
  def create_load_or_update_vocab_files(self, data_config, save_dir, filenames=None, update_only=False):
      ...
      for filename in filenames:
        with open(filename, 'r') as f:
          for line in f:
            line = line.strip()
            if line:
              split_line = line.split('schil') # split separator changed to 'schil' for fakenews
    ...
    for v in vocabs_index.keys():
      ...
      if 'label_components' in self.data_config[v]: # joint_pos_predicate has gold_pos and predicate label_components
        joint_vocab_map = vocabs[vocabs_index[v]] # map of joint_pos_predicate tokens and counts
        label_components = self.data_config[v]['label_components'] # [gold_pos,predicate]
        component_keys = [vocabs[vocabs_index[d]].keys() for d in label_components] # list of pos and predicates
